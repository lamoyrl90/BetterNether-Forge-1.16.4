package redd90.betternether.structures;
		...
		for (int x = 0; x < 16; x++) {
			...
			for (int z = 0; z < 16; z++) {
				...
				if (rigid < 0.015) {
					...
					B_POS.setY(height);
					if (world.isAirBlock(B_POS) && world.getBlockState(B_POS.move(Direction.DOWN)).hasOpaqueCollisionShape(world, B_POS) && isHeightValid(world, B_POS.up())) {
