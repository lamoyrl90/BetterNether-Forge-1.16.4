...
import redd90.betternether.blocks.BlockAnchorTreeVine;
import redd90.betternether.blocks.BlockPlantWall;
...
public class StructureAnchorTreeRoot implements IStructure {
	...
	public void generate(IServerWorld world, BlockPos pos, Random random) {
		...
		if (count < 3) count = 3;
		if ((count & 1) == 0) count++;
		POS.setPos(pos.getX() - dx * size, pos.getY() + 10, pos.getZ() - dz * size);
		BlockPos start = POS.up(BlocksHelper.upRay(world, POS, 64));
		if (start.getY() < pos.getY()) start = POS.setPos(start).add(0, 10, 0).toImmutable();
		POS.setPos(pos.getX() + dx * size, pos.getY() + 10, pos.getZ() + dz * size);
		BlockPos end = POS.up(BlocksHelper.upRay(world, POS, 64));
		if (end.getY() < pos.getY()) end = POS.setPos(end).add(0, 10, 0).toImmutable();
		...
		for (BlockPos bpos : BLOCKS) {
			...
			if (random.nextInt(8) == 0 && !BLOCKS.contains(bpos.north()) && world.isAirBlock(bpos.north()))
				BlocksHelper.setWithoutUpdate(world, bpos.north(), state.with(BlockPlantWall.FACING, Direction.NORTH));
			if (random.nextInt(8) == 0 && !BLOCKS.contains(bpos.south()) && world.isAirBlock(bpos.south()))
				BlocksHelper.setWithoutUpdate(world, bpos.south(), state.with(BlockPlantWall.FACING, Direction.SOUTH));
			if (random.nextInt(8) == 0 && !BLOCKS.contains(bpos.east()) && world.isAirBlock(bpos.east()))
				BlocksHelper.setWithoutUpdate(world, bpos.east(), state.with(BlockPlantWall.FACING, Direction.EAST));
			if (random.nextInt(8) == 0 && !BLOCKS.contains(bpos.west()) && world.isAirBlock(bpos.west()))
	...
	private void buildLine(List<BlockPos> blocks, double radius) {
		for (int i = 0; i < blocks.size() - 1; i++) {
			...
			int count = (int) Math.ceil(Math.sqrt(b.distanceSq(a)));
	...
	private List<BlockPos> lineParable(BlockPos start, BlockPos end, int count, Random random, double range) {
		...
		int middle = count / 2;
		result.add(start);
		double size = Math.sqrt(start.distanceSq(end)) * 0.8;
		for (int i = 1; i < max; i++) {
