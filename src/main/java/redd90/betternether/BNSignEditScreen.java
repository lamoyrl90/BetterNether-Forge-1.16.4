...
import redd90.betternether.blocks.BNSign;
...
public class BNSignEditScreen extends Screen {
	...
	public BNSignEditScreen(BNSignTileEntity sign) {
	...
	protected void init() {
		...
		}, TextInputUtil.getClipboardTextSupplier(this.minecraft), TextInputUtil.getClipboardTextSetter(this.minecraft), (string) -> {
	...
	public void removed() {
		...
		if (clientPlayNetworkHandler != null) {
			clientPlayNetworkHandler.sendPacket(new CUpdateSignPacket(this.sign.getPos(), this.text[0], this.text[1], this.text[2], this.text[3]));
		}
	...
	public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
		...
		for (m = 0; m < this.text.length; ++m) {
			...
			if (string2 != null) {
				...
				this.minecraft.fontRenderer.func_238411_a_(string2, n, (float) (m * 10 - this.text.length * 5), i, false,
		...
		for (m = 0; m < this.text.length; ++m) {
			...
			if (string2 != null && m == this.currentRow && j >= 0) {
				int r = this.minecraft.fontRenderer
						.getStringWidth(string2.substring(0, Math.max(Math.min(j, string2.length()), 0)));
