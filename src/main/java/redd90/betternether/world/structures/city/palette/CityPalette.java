package redd90.betternether.world.structures.city.palette;
...
import redd90.betternether.blocks.BlockPottedPlant;
import redd90.betternether.blocks.BlockPottedPlant.PottedPlantShape;
import redd90.betternether.blocks.BlockSmallLantern;
...
public class CityPalette {
	...
	private Block getRandomBlock(BlockState state, List<Block> list) {
		...
		String seed = ForgeRegistries.BLOCKS.getKey(state.getBlock()).getPath();
	...
	private BlockState copyWall(BlockState source, Block block) {
		BlockState state = block.getDefaultState()
				.with(WallBlock.WALL_HEIGHT_EAST, source.get(WallBlock.WALL_HEIGHT_EAST))
				.with(WallBlock.WALL_HEIGHT_NORTH, source.get(WallBlock.WALL_HEIGHT_NORTH))
				.with(WallBlock.WALL_HEIGHT_SOUTH, source.get(WallBlock.WALL_HEIGHT_SOUTH))
				.with(WallBlock.WALL_HEIGHT_WEST, source.get(WallBlock.WALL_HEIGHT_WEST))
	...
	private BlockState copyGate(BlockState source, Block block) {
		BlockState state = block.getDefaultState()
				...
				.with(FenceGateBlock.HORIZONTAL_FACING, source.get(FenceGateBlock.HORIZONTAL_FACING));
		return state;
	}

	private BlockState copyPillar(BlockState source, Block block) {
		BlockState state = block.getDefaultState()
				.with(RotatedPillarBlock.AXIS, source.get(RotatedPillarBlock.AXIS));
		return state;
	}
	...
	private BlockState copyLanternWall(BlockState source, Block block) {
		if (source.getBlock() instanceof BlockSmallLantern && !(block instanceof BlockSmallLantern)) {
			...
			if (block instanceof WallTorchBlock) {
				return state.with(WallTorchBlock.HORIZONTAL_FACING, facing);
			}
			return state;
		}
		return source;
	...
	private BlockState copyTrapdoor(BlockState source, Block block) {
		BlockState state = block.getDefaultState()
				.with(TrapDoorBlock.HORIZONTAL_FACING, source.get(TrapDoorBlock.HORIZONTAL_FACING))
